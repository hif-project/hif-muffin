...
- **Hierarchical VHDL does not round-trip into non-empty VHDL.** `hif2vhdl` exits 0 but writes a zero-byte file for the *parent* design unit of a hierarchical design; the child is emitted correctly. Reproduced on plain `vhdl2hif` output as well as on the same HIF after `muffin --instrument`, so it is a backend gap rather than an instrumentation defect — [hif-backend#27](https://github.com/hif-project/hif-backend/issues/27). Consequence: hierarchical instrumentation can be verified by simulation on the Verilog path only.

  The Verilog half of this limitation is resolved. `verilog2hif -s` → `muffin --instrument` → `hif2verilog` now regenerates a hierarchy that compiles and simulates, and the `hierarchical_behavioral` test checks golden behaviour and four faults against exact oracles. Previously the regenerated parent declared its instance-connected nets as `reg` while binding them to child instance *output* ports, which Verilog forbids, so Icarus rejected the design at elaboration ([hif-backend#26](https://github.com/hif-project/hif-backend/issues/26), fixed). `.ci/pinned-refs.env` pins the backend commit carrying that fix.
