...
- **Structure-preserving Verilog (`verilog2hif -s`) does not round-trip into compilable Verilog.** With `-s` the module hierarchy is kept (which is what makes Muffin's per-instance activation-port wiring reachable from Verilog at all), but `hif2verilog` then regenerates the parent's internal connection nets as `reg` while still binding them to child instance *output* ports — which Verilog forbids, so Icarus rejects the result at elaboration. Confirmed with Muffin absent from the pipeline entirely (`verilog2hif -s` → `hif2verilog`), so it is a frontend/backend round-trip gap, not an instrumentation defect. The equivalent VHDL path has its own gap: `hif2vhdl` exits 0 but writes a zero-byte file for the parent unit. Consequence: hierarchical instrumentation can be verified structurally (see the `hierarchical_wiring` test) but not yet by simulation.
