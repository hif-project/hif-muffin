...
- Added `examples/and2`, a step-by-step tutorial that takes a three-line AND gate through the full `verilog2hif` -> `muffin` -> `hif2verilog` pipeline and simulates it with Icarus Verilog, so `muffinMutPort = 0/1/2` (golden, `y` stuck-at-0, `y` stuck-at-1) can be observed against a golden reference. Documentation only; no implementation or test-suite changes.
